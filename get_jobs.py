...
columns = [
    'Title', 'Company', 'Location', 'Short Description', 'Link', 'Search Term',
    'Source'
]
...
def indeed_jobs_parser(soup, query):
    ...
    for item in soup.find_all(
            name='div', attrs={'class': re.compile('.*row.*')}):
        ...
        for span in item.find_all(
                name='span', attrs={'itemprop': 'addressLocality'}):
            try:
        ...
        if not Jobs.query.filter(
                and_(Jobs.title == job_item['Title'], Jobs.description ==
                     job_item['Short Description'])).first():
            ...
            db.session.add(
                Jobs(
                    ...
                    link=job_item['Link'],
                    search_term=(job_item['Search Term']).replace("+", " ")
                    .title(),
                    source=job_item['Source']))

            db.session.commit()
        else:
            duplicate = Jobs.query.filter(
                and_(Jobs.title == job_item['Title'], Jobs.description ==
                     job_item['Short Description'])).first()
...
queries = [
    'assistant+bilingue', 'assistant+evenementiel', 'assistant+marketing',
    'marketing+operationnel', 'marketing+junior', 'english', 'marketing',
    'event+assistant'
]
...
for query in queries:
    for result_batch in range(0, indeed_max_results, 10):
        payload = {
            'q': query + '+-' + indeed_compiled_exclude,
            'l': 'Lyon',
            'start': result_batch
        }
        request = Request(
            'GET', 'https://www.indeed.fr/emplois', params=payload)
